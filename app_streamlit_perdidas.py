...
def main():
    ...
    tabla_mostrar = resumen_inst.copy()

    # 1. Eliminar columna AÑO
    tabla_mostrar = tabla_mostrar.drop(columns=["AÑO"], errors="ignore")

    # 2. Eliminar columna formateada que no quieres mostrar
    tabla_mostrar = tabla_mostrar.drop(columns=["PERDIDA_TOTAL_CLP_FMT"], errors="ignore")

    # 3. Opcional: renombrar alguna otra columna para mostrarla mejor
    tabla_mostrar = tabla_mostrar.rename(columns={
        "PERDIDA_TOTAL_CLP": "Pérdida total (CLP)"})
